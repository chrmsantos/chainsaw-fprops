...
'================================================================================
' MÓDULO PRINCIPAL DE RETIFICAÇÃO DE PROPOSITURAS
'================================================================================
' Requisitos:
...
'--------------------------------------------------------------------------------
Private Const BACKUP_BASE_PATH As String = "Documents\DocsOriginaisBkp\"
Private Const HEADER_IMAGE_PATH As String = "Documents\Configurations\DefaultHeader.png"
...
Private Function ProcessDocument(doc As Document) As Integer
    ...
    doc.Content.Text = RemoveBlankLines(doc.Content.Text)
    editCount = editCount + 1

    ' Formata a primeira linha
    If doc.Paragraphs.Count > 0 Then
        FormatParagraph doc.Paragraphs(1), True, wdAlignParagraphCenter
        editCount = editCount + 1
    End If
...
Private Function ClearDocumentFormatting(doc As Document) As Integer
    ...
    ClearDocumentFormatting = 1 ' Considera como uma edição realizada
End Function
...
'--------------------------------------------------------------------------------
Private Function RemoveExtraSpaces(doc As Document) As Integer
    On Error Resume Next
...
'--------------------------------------------------------------------------------
' Função: RemoveBlankLines
' Remove linhas em branco de um intervalo de texto e retorna o texto limpo.
'--------------------------------------------------------------------------------
Private Function RemoveBlankLines(text As String) As String
    Dim lines As Variant
    Dim cleanedLines As Collection
    Dim line As Variant

    Set cleanedLines = New Collection
    ...
    RemoveBlankLines = Join(Application.Transpose(cleanedLines), vbCrLf)
End Function

'--------------------------------------------------------------------------------
' Função: ReplaceTerms
' Substitui termos em um texto com base em um dicionário de substituições.
'--------------------------------------------------------------------------------
Private Function ReplaceTerms(text As String, replacements As Variant) As String
    Dim i As Integer
    For i = LBound(replacements) To UBound(replacements)
        text = Replace(text, replacements(i)(0), replacements(i)(1), , , vbTextCompare)
    Next i
    ReplaceTerms = text
End Function

'--------------------------------------------------------------------------------
' Função: FormatParagraph
' Aplica formatação a um parágrafo com base em configurações fornecidas.
'--------------------------------------------------------------------------------
Private Function FormatParagraph(para As Paragraph, isBold As Boolean, alignment As WdParagraphAlignment, Optional leftIndent As Double = 0) As Paragraph
    With para.Range
        .Font.Bold = isBold
        .ParagraphFormat.Alignment = alignment
        .ParagraphFormat.LeftIndent = leftIndent
    End With
    Set FormatParagraph = para
End Function

'--------------------------------------------------------------------------------
' SUBROTINA: FixDocumentFormatting
' Finalidade: Corrige a formatação do documento ativo, removendo espaços em branco,
' linhas em branco e ajustando quebras de página.
'--------------------------------------------------------------------------------
Sub FixDocumentFormatting()
    ...
    ' Verifica se há um documento ativo
    If Documents.Count = 0 Then
        MsgBox "Nenhum documento está aberto. Por favor, abra um documento antes de executar a correção de formatação.", _
               vbExclamation, "Documento não encontrado"
        Exit Sub
    End If
    ...
    editCount = editCount + RemoveExtraSpaces(doc)
    editCount = editCount + RemoveExtraPageBreaks(doc)
    ...
    ' Mensagem de conclusão
    MsgBox "Correção de formatação concluída com sucesso!" & vbCrLf & _
           "Número de edições realizadas: " & editCount, vbInformation, "Formatação Corrigida"
    Exit Sub

ErrorHandler:
    ' Tratamento de erros
    MsgBox "Erro " & Err.Number & ": " & Err.Description, vbCritical, "Erro na Correção de Formatação"
    Application.ScreenUpdating = True
End Sub
